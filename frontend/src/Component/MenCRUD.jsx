...
const MenCRUD = () => {
  ...
  const handleAddSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('http://192.168.1.109:3000/api/admin/men', newItem, { headers: { Authorization: `Bearer ${token}` } });
      setMen([...men, res.data]);
      setNewItem({
        name: '',
        category: '',
  ...
  const handleSave = async (id) => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.put(`http://192.168.1.109:3000/api/admin/men/${id}`, editedItem, { headers: { Authorization: `Bearer ${token}` } });
      const updatedMen = [...men];
      updatedMen[editingIndex] = res.data;
      setMen(updatedMen);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setMen(men.filter(item => item._id !== id));
    } catch (err) {
      console.error(err.message);
    }
  };

...
const KidsCRUD = () => {
  ...
  const handleAddSubmit = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await axios.post('http://192.168.1.109:3000/api/admin/kids', newItem, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSave = async (id) => {
    const token = localStorage.getItem('token');
    try {
      const res = await axios.put(`http://192.168.1.109:3000/api/admin/kids/${id}`, editedItem, { headers: { Authorization: `Bearer ${token}` } });
      const updatedKids = [...kids];
      updatedKids[editingIndex] = res.data;
      setKids(updatedKids);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setKids(kids.filter(item => item._id !== id));
    } catch (err) {
      console.error(err.message);
    }
  };
